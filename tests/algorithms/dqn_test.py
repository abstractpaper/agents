...
def transitions(agent):
    ...
    transitions, _, _ = agent.replay_buffer.sample(agent.batch_size)
    return transitions
...
def test_select_action_random(agent):
    obs = agent.env.reset()
    actions = []
...
def test_select_action_greedy(agent):
    obs = agent.env.reset()
...
def test_load_replay_buffer(agent):
    assert len(agent.replay_buffer) == 0
    agent.load_replay_buffer()
    assert len(agent.replay_buffer) > 0

def test_state_action_values(agent, transitions):
    ...
    for i in range(agent.batch_size):
        actions = agent.policy_net(torch.Tensor(batch.state[i]), torch.Tensor(batch.mask[i]))
        x = actions.squeeze()[batch.action[i]]
        y = state_action_values.squeeze()[i]
        # float comparison up to 3 decimal points
        assert abs(x.item()-y.item()) < 1e-3

def test_expected_state_action_values(env, agent, transitions):
    batch = Transition(*zip(*transitions))
...
def test_double_dqn_expected__state_action_values(env, agent, transitions):
    agent.double = True
    batch = Transition(*zip(*transitions))
    expected_state_action_values = agent.expected_state_action_values(batch)
    assert len(expected_state_action_values) == agent.batch_size